...
import {getGBP} from '../services/currencies';
...
export default function Screen()
{
    ...
    const GBP = conversion
    const BRL = currentValue
    let valueExchange;
    if(BRL > 0){
        valueExchange = BRL * parseInt(GBP);
    }else{
        valueExchange =0;
    }
    const updateCurrency = async () =>{
        setLoading(true);
        const dolar = await getGBP()
        setLoading(false);
    ...
    return (
        <View style={styles.boxCurrency}>
            ...
            <Image source={require('../assets/gbp.png')} style={styles.img}/>
            <View style={styles.boxCurrency}>
          
            <Text style={styles.textValue}> 1 GBP = R${currentValue} </Text>
            </View>
            <Pressable onPress={updateCurrency}>
                <Text style={styles.textBtn}>Update Currency</Text>
            </Pressable>
            <Text style={styles.titleConversion}>Convert GBP to REAL</Text>
